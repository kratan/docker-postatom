...
#Add User
useradd -g xpra -u 5000 -ms /bin/bash ${USERNAME}
...
su - ${USERNAME} -c 'Xorg :11 -keeptty -novtswitch -sharevts vt'${USEVT}' & (cd /postAtom/ && XPRA_PASSWORD='${XPRA_PASSWORD}' xpra start :11 −−start−after−connect=yes --bind-tcp=0.0.0.0:'${XPRAPORT}' --auth=env --ssl=on --ssl-cert=/home/'${USERNAME}'/gpunode.crt --ssl-key=/home/'${USERNAME}'/gpunode.key --no-clipboard --no-pulseaudio --start-child="./postAtom" --exit-with-child --no-printing --no-speaker --no-cursors --dbus-control=no --dbus-proxy=no --use-display --no-daemon)'
